package com.netease.lowcode.lib.file;
...
@Component("itextpdfFileConnectorUtils")
public class FileConnectorUtils {
     ...
     * @return base64String
     */
    public InputStream fileDownloadV2(String fileUrl) {
        try {
    ...
    public String dealPath(String path) {
        ...
        path = path.startsWith("/download") ? path.substring("/download".length()) :
                path.substring(path.indexOf(UPLOAD_API_PATH_PREFIX) + UPLOAD_API_PATH_PREFIX.length());
        if (path.startsWith("/")) {
